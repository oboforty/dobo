...
def check_valid_recur(bst: RedBlackTree, node: Node) -> int:
    ...
    if node.is_null() or (node.left.is_null() and node.right.is_null()):
        return

    check_valid_recur(bst, node.left)
    check_valid_recur(bst, node.right)
...
def test_long() -> None:
    ...
    resp = bst.search(714880750)
    assert resp is not None
    t2 = time.time()
    print("Test took:", round(t2-t1), 'sec')
